...
def build_graph(
    ...
    if maxun_readonly:
        ...
        chart_tools = []
        if engine is None:
            raise ValueError("Maxun read-only profile requires a resolved workspace engine")
    ...
    if maxun_readonly:
        # A Maxun turn is a completed answer, not a chart workflow.
